...
import { Alert, Badge, Descriptions, Typography, message } from "antd";
const { Text } = Typography;
import { useRouter } from "next/router";
import React from "react";

const ViewOneMovingOwner = ({ moving, idMoving, fechaProgramadaValida }) => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      messageApi.success(send.message, 3, () => router.push("/moving/list"));
      return true;
    } catch (error) {
      console.log(error);
      return false;
    ...
    },
    {
      ...
      children:
        moving.moving_response.length > 0 ? (
          ...
          <MovingReplyOwnerFormComponent onSubmit={onSubmit} />
        ) : (
...
export async function getServerSideProps(context) {
  try {
    ...
    const getData = await MovingController.apiSSRGetOneMovingByOwner(
      getCookies,
      idMoving
    );
...
export default ViewOneMovingOwner;
